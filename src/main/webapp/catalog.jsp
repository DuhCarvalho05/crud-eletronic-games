				...
				<c:otherwise>
					...
					<ul style="list-style: none; padding: 0; margin: 0">
						<c:forEach var="cat" items="${ categories }">
							<c:choose>
								<c:when test="${ category.name == cat.name }">
									<li style="margin-bottom: 10px; background-color: #fff; border-radius: 5px" class="category-link d-flex">
										<a
										style="text-decoration: none; color: #333; padding: 0px 4px; width: 100%"
										class="text-capitalize fw-medium"
										href="${pageContext.request.contextPath}/catalog/${ cat.name }">
											${ cat.name } </a>
									</li>
								</c:when>
								<c:otherwise>
						...
						<c:forEach var="game" items="${ games }">
							<div>
								<div style="border: 1px solid #27272a;"
									class="c-card d-flex rounded position-relative">
									<a class="d-flex w-100 p-2" href="${pageContext.request.contextPath}/details/${ game.id }"> <img
										style="opacity: 0.8;" class="w-100 rounded"
